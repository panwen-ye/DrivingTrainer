...
7. 给路线添加至少两个“考核项目”，切换到“模拟考试播报”，依次进入触发范围；确认只播报项目口令，不播报训练口诀、开始、暂停、继续或结束提示。
8. 暂停模拟考试后进入下一个考核点范围，确认不播报；继续后重新进入范围，确认按预设顺序恢复。
9. 打开记录详情，确认节点结果正确。
10. 从 Apple 地图复制一条真实驾驶路线链接，导入后在地图上核对，并确认原训练提示点和考核项目未丢失。
...
4. 切换模拟考试模式后，Watch 仅显示当前考核项目口令并产生通知触觉，不显示训练口诀。
5. 临时关闭 Watch 蓝牙或离开连接范围，确认 iPhone 训练不崩溃。
6. 恢复连接后开始下一个节点，确认摘要重新同步。
...
- iPhone 能安装、冷启动和重新启动。
- 真实 GPS 录制、训练提示点、考核项目保存及两种模式的独立触发通过。
- 模拟考试模式的扬声器与 Watch 均只输出预设考核项目口令。
